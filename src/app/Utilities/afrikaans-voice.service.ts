...
export class AfrikaansVoiceService {
  ...
  private populateVoices() {
    this.voices = speechSynthesis.getVoices().filter(voice =>
      voice.lang.startsWith('af')
    );
  }

  private loadStoredVoice() {
    const storedVoiceId = localStorage.getItem('selectedVoiceAfrikaans');
  ...
  setSelectedVoice(voice: SpeechSynthesisVoice | null) {
    ...
    localStorage.setItem('selectedVoiceAfrikaans', voice ? this.getVoiceId(voice) : '');
  }
