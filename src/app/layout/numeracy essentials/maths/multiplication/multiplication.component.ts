...
export class MultiplicationComponent implements OnInit, OnDestroy {
  ...
  generateProblems() {
    ...
    try {
      for (let i = 0; i < 10; i++) {
        ...
        newProblems.push({ numbers, product, missingIndex });
      }
  ...
  checkAnswer(problemIndex: number, position: string) {
    ...
    const correctAnswer = position === 'product'
      ? currentProblem.product
    ...
    if (userAnswer === correctAnswer) {
      ...
      setTimeout(() => {
        if (problemIndex < this.problems.length - 1) {
          ...
          const nextInputId = `input_${problemIndex + 1}_${nextProblem.missingIndex === nextProblem.numbers.length ? 'product' : 'num' + (nextProblem.missingIndex + 1)}`;
  ...
  private formatProblemForSpeech(problem: Problem): string {
    const { numbers, product, missingIndex } = problem;
    if (missingIndex === numbers.length) {
      return `${numbers.join(' multiplied by ')} equals what?`;
    } else {
      const questionParts: (number | string)[] = [...numbers];
      questionParts[missingIndex] = 'what number';
      return `${questionParts.join(' multiplied by ')} equals ${product}`;
    }
  }
  ...
  getMaxLength(problemIndex: number, block: string): number {
    const currentProblem = this.problems[problemIndex];
    if (block === 'product') {
      return currentProblem.product.toString().length;
    } else {
  ...
  getBlocks(problemIndex: number): string[] {
    const currentProblem = this.problems[problemIndex];
    return [...currentProblem.numbers.map((_, index) => 'num' + (index + 1)), 'product'];
  }
  ...
  getBlockValue(problemIndex: number, blockIndex: number): number {
    const problem = this.problems[problemIndex];
    return blockIndex === problem.numbers.length ? problem.product : problem.numbers[blockIndex];
  }
}
