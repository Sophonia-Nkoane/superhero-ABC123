...
export class SubtractionComponent implements OnInit, OnDestroy {
  ...
  generateProblems() {
    ...
    try {
      for (let i = 0; i < 10; i++) {
        ...
        const sum = numbers.reduce((acc, curr) => acc - curr); // Use subtraction for the sum
  ...
  private generateNumbersForLowerLevels(): number[] {
    ...
    } while (num1 - num2 < 0 || num1 - num2 > 10); // Ensure the result is valid for subtraction
    return [num1, num2];
  }
  ...
  private generateNumbersForLevelThree(): number[] {
    let num1, num2;
    do {
      num1 = this.generateRandomNumber(10, 100);
      num2 = this.generateRandomNumber(1, num1); // Ensure num2 is less than or equal to num1
    } while (num1 - num2 < 1); // Ensure the result is positive
    return [num1, num2];
  }
  ...
  private generateNumbersForHigherLevels(): number[] {
    let numbers: number[];
    do {
      numbers = Array.from({ length: 5 }, () => this.generateRandomNumber(1, 100));
    } while (numbers.reduce((acc, curr) => acc - curr) < 0); // Ensure the result is valid for subtraction
    return numbers;
  }
  ...
  private formatProblemForSpeech(problem: Problem): string {
    ...
    if (missingIndex === numbers.length) {
      return `${numbers.join(' minus ')} equals what?`;
    } else {
      const questionParts: (number | string)[] = [...numbers];
      questionParts[missingIndex] = 'what number';
      return `${questionParts.join(' minus ')} equals ${sum}`;
    }
  }
